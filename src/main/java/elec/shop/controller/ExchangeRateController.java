...
public class ExchangeRateController {
    ...
    @PostMapping("/accounts/changeIds/exchange-rate")
    @ApiOperation("批量更新指定账户的指定汇率信息")
    @DataSource(DataSourceType.MASTER)
    public Result<Object> changeIdsCurrencyAccounts(@RequestBody ExchangeRateIdsDTO exchangeRateIdsDTO) {
        try {
            if (!Arrays.asList("USD", "EUR", "GBP", "JPY", "CNY").contains(exchangeRateIdsDTO.getCurrency())) {
                return Result.fail().message("不支持的货币类型");
            }
            Boolean result = accountBalanceService.changeIdsCurrencyAccounts(exchangeRateIdsDTO);
            return Result.ok(result);
        } catch (Exception e) {
            return Result.fail().message("获取账户信息失败：" + e.getMessage());
        }
